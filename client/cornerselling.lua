        ...
        if stealingPed ~= nil and stealData ~= nil then
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    DrawText3D(pedpos.x, pedpos.y, pedpos.z, QBCore.Shared._U(Locales, "client_cornerselling_createthread_drawtext3d"))
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                QBCore.Functions.Notify(QBCore.Shared._U(Locales, "client_cornerselling_function_SellToPed_notify_1", bagAmount, availableDrugs[drugType].label), 'error')        
                stealingPed = ped
                stealData = {
